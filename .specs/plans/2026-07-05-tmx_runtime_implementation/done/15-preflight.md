...
- [x] Orchestrate load → `kind` dispatch → reference resolution → directory assembly → desugar → validate → capability check via the `SourceLoader`/`ReferenceResolver`/`SchemaValidator` ports.
...
- [x] Enforce the preflightable limits as `Validation` errors (`too_many_tasks`, literal `fanout_too_wide`, `json_too_deep`, `concurrency_too_high`, `too_many_hook_tasks`) and the structural checks (`missing_task_name`, duplicate-name `ResolutionError`); emit the newer-spec compatibility warning and reject an unknown construct as `Validation`.
- [x] Compute the `CapabilitySet` (recursing into `map`/`eval` inner tasks, `eval` scorer kinds, hook bodies, provider methods) and return `EnvironmentError` (`missing_capability`) when a required adapter is absent or a denying stub.

## Definition of done

- [x] A passing preflight yields a `ResolvedFlow` (ordered `Vec<Task>`, resolved env/context) and a `CapabilitySet`; a directory with one malformed task aborts before any task executes.
- [x] Over-limit counts/widths/depths and a nameless or duplicate-named task are rejected at preflight, and a Flow needing an unwired capability returns `missing_capability` naming the port and task type (negative space).
- [x] Meets the repo definition of done (tests, lint/format, named-constant limits — see plan.md baseline).
- [x] Reviewable: preflight a directory of task files and confirm natural-filename ordering, and run the fail-fast tests for a malformed task and a missing capability.

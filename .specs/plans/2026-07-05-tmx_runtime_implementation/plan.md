...
**Status:** Done · **Layout:** kanban · **Date:** 2026-07-05 · **Owner:** Ant Stanley · **Source spec:** [`.specs/`](../../00-overview.md) (repo-wide canonical set)

> **Build complete.** All 32 planned task packages are in `done/`, each proven correct + complete by a separate verifier and landed on the `tmx-runtime` bookmark (one commit per task). One post-hoc task — **33 · wire map/eval dispatch** — was discovered during the build and added to close a decomposition gap (the `map`/`eval` task types were built but never wired into the runner's dispatch); it is also `done/`. Known remaining limitations are recorded in the build report (the `as:` map element-binding alias is schema-accepted but not honoured by the interpolator; reference-form context/environment flows fail-closed at the engine re-load; both fail loudly and are safe).
